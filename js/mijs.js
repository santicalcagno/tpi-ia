    ...
    if(x < max_inputs) {
      ...
      for (var i = 0; i < inputs.length; i++) {
        if (!inputs[i]) {
          inputs[i] = 1;
          ix = i + 1;
          $(form_operandos).append('<div class="form-group"><label for="op' + ix + '" class="control-label col-xs-2">Operando</label><div class="col-xs-2"><input type="text" class="form-control pal" id="op' + ix + '" maxlength="10"></div><button class="btn btn-danger remove_field" type="button"><span class="glyphicon glyphicon-remove"></span></button></div>');
          break;
        }
      }
    }
